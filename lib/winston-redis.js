...
Redis.prototype.streamPubsub = function (options) {
  ...
  stream.destroy = function () {
    ...
    self.subscription[channel].removeListener('pmessage', message);

    if (!self.subscription[channel].listeners('pmessage').length) {
      try {
  ...
  if (!this.subscription[channel]) {
    ...
    this.subscription[channel].psubscribe(channel);
  }
  ...
  this.subscription[channel].on('pmessage', message);
...
Redis.prototype.streamPubsub = function (options) {
  ...
  if (!this.subscription) {
    ...
    this.subscription.on('pmessage', function (pattern, channel, message) {
      ...
      if (!listeners) return; // should never happen
